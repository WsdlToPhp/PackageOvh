...
class SqlpriveCronDel extends AbstractStructBase
{
     ...
     * Set session value
     * @param string $session
     * @return \PayPal\StructType\SqlpriveCronDel
     */
    ...
    public function setCronId($cronId = null)
    {
        ...
        if (!is_null($cronId) && !is_string($cronId)) {
            throw new \InvalidArgumentException(sprintf('Invalid value, please provide a string, "%s" given', gettype($cronId)), __LINE__);
        }
        $this->cronId = $cronId;
        return $this;
    }
