...
class MyArrayOfTelephonyLineSwitchPossibilityPriceStructType extends AbstractStructArrayBase
{
    ...
    public static function validateItemForArrayConstraintsFromSetItem(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $myArrayOfTelephonyLineSwitchPossibilityPriceStructTypeItemItem) {
            ...
            if (!$myArrayOfTelephonyLineSwitchPossibilityPriceStructTypeItemItem instanceof \Ovh\StructType\TelephonyLineSwitchPossibilityPriceStruct) {
                $invalidValues[] = is_object($myArrayOfTelephonyLineSwitchPossibilityPriceStructTypeItemItem) ? get_class($myArrayOfTelephonyLineSwitchPossibilityPriceStructTypeItemItem) : sprintf('%s(%s)', gettype($myArrayOfTelephonyLineSwitchPossibilityPriceStructTypeItemItem), var_export($myArrayOfTelephonyLineSwitchPossibilityPriceStructTypeItemItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The item property can only contain items of type \Ovh\StructType\TelephonyLineSwitchPossibilityPriceStruct, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
    ...
    public function addToItem(\Ovh\StructType\TelephonyLineSwitchPossibilityPriceStruct $item)
    {
        ...
        if (!$item instanceof \Ovh\StructType\TelephonyLineSwitchPossibilityPriceStruct) {
            throw new \InvalidArgumentException(sprintf('The item property can only contain items of type \Ovh\StructType\TelephonyLineSwitchPossibilityPriceStruct, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->item[] = $item;
        return $this;
    }
