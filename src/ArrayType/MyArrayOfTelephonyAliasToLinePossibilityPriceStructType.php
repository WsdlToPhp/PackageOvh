...
class MyArrayOfTelephonyAliasToLinePossibilityPriceStructType extends AbstractStructArrayBase
{
    ...
    public static function validateItemForArrayConstraintsFromSetItem(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $myArrayOfTelephonyAliasToLinePossibilityPriceStructTypeItemItem) {
            ...
            if (!$myArrayOfTelephonyAliasToLinePossibilityPriceStructTypeItemItem instanceof \Ovh\StructType\TelephonyAliasToLinePossibilityPriceStruct) {
                $invalidValues[] = is_object($myArrayOfTelephonyAliasToLinePossibilityPriceStructTypeItemItem) ? get_class($myArrayOfTelephonyAliasToLinePossibilityPriceStructTypeItemItem) : sprintf('%s(%s)', gettype($myArrayOfTelephonyAliasToLinePossibilityPriceStructTypeItemItem), var_export($myArrayOfTelephonyAliasToLinePossibilityPriceStructTypeItemItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The item property can only contain items of type \Ovh\StructType\TelephonyAliasToLinePossibilityPriceStruct, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
    ...
    public function addToItem(\Ovh\StructType\TelephonyAliasToLinePossibilityPriceStruct $item)
    {
        ...
        if (!$item instanceof \Ovh\StructType\TelephonyAliasToLinePossibilityPriceStruct) {
            throw new \InvalidArgumentException(sprintf('The item property can only contain items of type \Ovh\StructType\TelephonyAliasToLinePossibilityPriceStruct, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->item[] = $item;
        return $this;
    }
