...
class MyArrayOfSupportDomainStructType extends AbstractStructArrayBase
{
    ...
    public function setItem(array $item = array())
    {
        foreach ($item as $myArrayOfSupportDomainStructTypeItemItem) {
            ...
            if (!$myArrayOfSupportDomainStructTypeItemItem instanceof \Ovh\StructType\SupportDomainStruct) {
                throw new \InvalidArgumentException(sprintf('The item property can only contain items of \Ovh\StructType\SupportDomainStruct, "%s" given', is_object($myArrayOfSupportDomainStructTypeItemItem) ? get_class($myArrayOfSupportDomainStructTypeItemItem) : gettype($myArrayOfSupportDomainStructTypeItemItem)), __LINE__);
            }
        }
        $this->item = $item;
        return $this;
    ...
    public function addToItem(\Ovh\StructType\SupportDomainStruct $item)
    {
        ...
        if (!$item instanceof \Ovh\StructType\SupportDomainStruct) {
            throw new \InvalidArgumentException(sprintf('The item property can only contain items of \Ovh\StructType\SupportDomainStruct, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
        $this->item[] = $item;
        return $this;
    }
