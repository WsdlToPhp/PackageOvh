...
class MyArrayOfTelephonyBillingAccountSummaryLineStructType extends AbstractStructArrayBase
{
    ...
    public static function validateItemForArrayConstraintsFromSetItem(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $myArrayOfTelephonyBillingAccountSummaryLineStructTypeItemItem) {
            ...
            if (!$myArrayOfTelephonyBillingAccountSummaryLineStructTypeItemItem instanceof \Ovh\StructType\TelephonyBillingAccountSummaryLineStruct) {
                $invalidValues[] = is_object($myArrayOfTelephonyBillingAccountSummaryLineStructTypeItemItem) ? get_class($myArrayOfTelephonyBillingAccountSummaryLineStructTypeItemItem) : sprintf('%s(%s)', gettype($myArrayOfTelephonyBillingAccountSummaryLineStructTypeItemItem), var_export($myArrayOfTelephonyBillingAccountSummaryLineStructTypeItemItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The item property can only contain items of type \Ovh\StructType\TelephonyBillingAccountSummaryLineStruct, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
    ...
    public function addToItem(\Ovh\StructType\TelephonyBillingAccountSummaryLineStruct $item)
    {
        ...
        if (!$item instanceof \Ovh\StructType\TelephonyBillingAccountSummaryLineStruct) {
            throw new \InvalidArgumentException(sprintf('The item property can only contain items of type \Ovh\StructType\TelephonyBillingAccountSummaryLineStruct, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->item[] = $item;
        return $this;
    }
